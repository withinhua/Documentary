...
function parseHash(hash: string): RouterState {
  ...
  let route: AppRoute = (pathParts[0] || "studio") as AppRoute;
  ...
  if (route === "studio" && pathParts[1]) {
    params.production = decodeURIComponent(pathParts[1]);
  }
  ...
  return {
    route: validRoutes.includes(route) ? route : "studio",
    params,
  };
}
